...
export function useCaseHeroProgress({
  enabled = true,
  scrollFactor = 1 / 720,
  easing = 0.1,
  touchTravel = 280,
  touchToggleThreshold = 84
} = {}) {
    ...
    viewportHeight <= 560
      ...
      : viewportHeight <= 768
        ...
        : viewportHeight <= 1024
          ...
          ? 920
          : 1240;
  const fallbackFooterHeight = viewportHeight <= 700 ? 340 : 300;
  const measuredFooterHeight = footerHeight || fallbackFooterHeight;
  const viewportAllowance = viewportHeight - measuredFooterHeight;
  const baseMediaHeight = Math.max(
    220,
    Math.min(maxBaseHeight, viewportAllowance > 0 ? viewportAllowance : maxBaseHeight)
  );
  ...
  const animateProgress = useCallback(() => {
    ...
    applyProgress(progressRef.current + difference * easing);
  ...
  useEffect(() => {
    ...
    const handleTouchStart = (event) => {
      const target = mediaWrapRef.current;
